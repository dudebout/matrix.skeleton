...
\def\pgf@matrix@create@skeleton{%
  ...
  \pgfscope%
	...
	% find nodes in the last row and set \ck@shift@y to maximum difference between text and south anchors
	\foreach \pgf@matrix@skel@node in \pgf@matrix@node@list {%
	  \ifx\pgf@matrix@skel@node\pgfutil@empty%
 	   \else%
		\edef\pgf@tmp@row{\expandafter\expandafter\expandafter\pgfutil@firstoftwo\csname pgf@matrix@node@location@\pgf@matrix@skel@node \endcsname}%
		\ifnum\pgf@tmp@row=\the\pgfmatrixcurrentrow%
			\pgf@process{\pgfpointanchor{\pgf@matrix@skel@node}{base}}%
			\pgf@yb=\pgf@y%
			\pgf@process{\pgfpointanchor{\pgf@matrix@skel@node}{south}}%
			\pgfmathsetlength\pgf@yb{\pgf@yb-\pgf@y}%
			\ifdim\pgf@yb>\ck@shift@y%
